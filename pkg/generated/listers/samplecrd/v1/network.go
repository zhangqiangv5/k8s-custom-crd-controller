...
type NetworkLister interface {
	...
	List(selector labels.Selector) (ret []*samplecrdv1.Network, err error)
...
type networkLister struct {
	listers.ResourceIndexer[*samplecrdv1.Network]
}

// NewNetworkLister returns a new NetworkLister.
func NewNetworkLister(indexer cache.Indexer) NetworkLister {
	return &networkLister{listers.New[*samplecrdv1.Network](indexer, samplecrdv1.Resource("network"))}
}

// Networks returns an object that can list and get Networks.
func (s *networkLister) Networks(namespace string) NetworkNamespaceLister {
	return networkNamespaceLister{listers.NewNamespaced[*samplecrdv1.Network](s.ResourceIndexer, namespace)}
}
...
type NetworkNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*samplecrdv1.Network, error)
